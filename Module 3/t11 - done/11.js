/*  Follow the instructions in t11.txt. Modify the program to open large image in a modal when <article> is clicked. (6p)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  for (let i = 0; i < picArray.length; i++) {
    ...
    // Add event listener to open modal with large image when article is clicked
    article.addEventListener('click', () => {
      const dialog = document.querySelector('dialog');
      const dialogImg = dialog.querySelector('img');
      const dialogSpan = dialog.querySelector('span');

      // Set the large image source and alt text
      dialogImg.src = data.image.large;
      dialogImg.alt = data.title;
